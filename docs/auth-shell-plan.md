...
> **Document status:** this is the **source of truth** for the upcoming work. Every other doc — including [noauth-plan.md](archive/noauth-plan.md), now implemented and squash-merged to `main` (`2b02a7b`, PR #12) — lives in [archive/](archive/) and **may be out of date**; where an archived doc disagrees with this one or with the code on `main`, this document and the code win.

Target: the **access-and-identity shell** that [noauth-plan.md](archive/noauth-plan.md) §2 promises and deliberately does not build. That rework is merged: the kernel is full-trust and exposes exactly three injection points — an `author` string, a `ScopeClaim[]` for read visibility, and a route/tool-name-shaped surface on which destructive ops can be gated. This plan defines the module that fills those points: it authenticates callers, maps each credential to an identity and a set of grants, enforces write and destructive-op policy, and forwards to the kernel — without the kernel ever learning that any of this exists.
