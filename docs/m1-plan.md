...
- `repos.delete(actor, slug)` — admin-gated; **idempotent**. Look up repo; if slug already system-namespaced, no-op. Else rename to `<system-sigil>deleted-{slug}-{uniquifier}` where the uniquifier is a short server-chosen suffix (proposal: base32 of `randomBytes(4)` — 6 chars). Documents inside untouched.
...
3. **Deletion-slug uniquifier for `repos.delete` / `users.delete`.** Proposal: 6-char base32 of `randomBytes(4)`. Enough entropy that a collision on the same slug base within a repo/user lifetime is essentially impossible; short enough not to bloat the visible system-namespace slug list.
