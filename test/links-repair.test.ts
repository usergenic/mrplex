...
function create(path: string, body: string) {
  return kernel.docs.create(actor, "notes", path, { frontmatter_raw: "", body });
}

async function bodyAt(path: string): Promise<string> {
  const v = await kernel.docs.get(actor, "notes", path);
  return v.body;
}
...
describe("links.stale", () => {
  ...
  it("reports a wikilink whose target moved", async () => {
    ...
    await kernel.docs.put(actor, "notes", target.version_id, "people/alice.md", {});
    const stale = await kernel.links.stale(actor, "notes");
    expect(stale).toHaveLength(1);
  ...
  it("does not report a pure recasing of the target (case-insensitive identity)", async () => {
    ...
    await kernel.docs.put(actor, "notes", target.version_id, "Horses.md", {});
    expect(await kernel.links.stale(actor, "notes")).toEqual([]);
  });
});
...
describe("mrplex links repair", () => {
  ...
  it("rewrites a wikilink destination without the .md extension", async () => {
    ...
    await kernel.links.repair(actor, "notes");
  ...
  it("rewrites multiple stale destinations in one document (right-to-left splice)", async () => {
    ...
    await kernel.docs.put(actor, "notes", a.version_id, "x/a.md", {});
    await kernel.docs.put(actor, "notes", b.version_id, "y/b.md", {});
  ...
  it("dry_run reports the plan but writes nothing", async () => {
    ...
    const res = await kernel.links.repair(actor, "notes", { dry_run: true });
    expect(res.dry_run).toBe(true);
    expect(res.repaired).toEqual([{ path: "note.md", edges: 1 }]);
    // Body unchanged; still stale.
    expect(await bodyAt("note.md")).toBe("[h](horses.md)");
    expect(await kernel.links.stale(actor, "notes")).toHaveLength(1);
  });
  ...
  it("every repair is a normal authored version (advances the chain)", async () => {
    ...
    await kernel.docs.put(actor, "notes", target.version_id, "animals/horses.md", {});
    await kernel.links.repair(actor, "notes");
    const after = await kernel.docs.get(actor, "notes", "note.md");
    expect(after.prev_version_id).toBe(note.version_id);
    expect(after.author.user).toBe("alice");
  });

  it("skips documents the caller cannot write (scope)", async () => {
    const target = await create("horses.md", "neigh");
    ...
    const scoped: Actor = {
      user_id: aliceId,
      admin: false,
      scopes: [{ repos: [repoId], read: ["**"], write: ["**", "!locked/**"] }],
    };
