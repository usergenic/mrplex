/**
 * kernel.query rank — the M4 semantic mode end-to-end.
 *
 * Assertions:
 *   • rank without a hook → rank_unavailable
 *   • rank orders documents by cosine distance to the query vector
 *   • rank ∩ filter ∩ text composes
 *   • out-of-scope and hidden/system docs are excluded from rank hits
 *   • docs without chunks under the queried model are absent from rank
...
describe("query — rank", () => {
  it("rank without a queryEmbed hook → rank_unavailable", async () => {
    ...
    try {
      await kernel.query(admin, { repo: "notes", rank: "anything" });
  ...
  it("system-namespace paths stay hidden from rank by default", async () => {
    ...
    const rows = await kernel.query(admin, { repo: "notes", rank: "hello" });
    expect(rows.length).toBe(0);
  ...
  it("hook error at query time → rank_unavailable with cause", async () => {
    ...
    try {
      await kernel.query(admin, { repo: "notes", rank: "x" });
      throw new Error("should have thrown");
    } catch (err) {
      expect((err as KernelError).code).toBe("rank_unavailable");
