...
function main(): void {
  ...
  try {
    ...
    const notes = storage.repos_create({ slug: "notes", created_at: isoAt(clock++) });
    ...
    for (const file of files) {
      ...
      const doc = storage.documents_create(notes.id);
    ...
    // Give welcome.md a second version so `docs history` has something interesting.
