...
--   target_raw  exactly what was written, normalized to repo-absolute;
--               anchor (#heading) preserved
...
-- document later appears at target_raw's path.
...
-- edges. Partial — dangling rows are served by the target_raw index below.
create index links_target_id_idx on links(target_id) where target_id is not null;

-- Dangling re-resolution lookups (§11.2): when a document appears at a
-- path, bind the waiting danglers whose target_raw names it.
